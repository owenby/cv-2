...
switch lower(FEATURE)    
    ...
    case 'colour histogram'
     ...
     case 'bag of sift'
        ...
        vocab_dir = strjoin(["vocabs/" int2str(VOCAB_SIZE) "clusters/"], '');
        vocab_path = strjoin([vocab_dir COLOUR_SPACE ".mat"], '');
        
        if ~exist(vocab_path, 'file')
            fprintf('No existing dictionary found. Computing one from training images\n')
            vocab = build_vocabulary(train_image_paths, VOCAB_SIZE, COLOUR_SPACE); %Also allow for different sift parameters
            save(vocab_path, 'vocab')
        else
            disp(['loading existing vocab from ' vocab_path]); 
            load(vocab_path);
        end
        
        % YOU CODE get_bags_of_sifts.m
        feats_dir = strjoin(["feats/" int2str(VOCAB_SIZE) "clusters/" int2str(SIFT_STEP) "step/"], '');
        feats_path = strjoin([feats_dir COLOUR_SPACE ".mat"], '');
        if ~exist(feats_path, 'file')
            train_image_feats = get_spatial_pyramid(train_image_paths, VOCAB_SIZE, COLOUR_SPACE, SIFT_STEP); %Allow for different sift parameters
            test_image_feats  = get_spatial_pyramid(test_image_paths); 
            save(feats_path, 'train_image_feats', 'test_image_feats')
        else
            disp(['loading existing features from ' feats_path]); 
            load(feats_path);
        end
        
      case 'spatial pyramid'
          % YOU CODE spatial pyramids method
          train_image_feats = get_spatial_optimised(train_image_paths, VOCAB_SIZE, COLOUR_SPACE, SIFT_STEP, 1);
          test_image_feats = get_spatial_optimised(test_image_paths, VOCAB_SIZE, COLOUR_SPACE, SIFT_STEP, 1);
end
...
switch lower(CLASSIFIER)    
    ...
    % Useful functions: pdist2 (Matlab) and vl_alldist2 (from vlFeat toolbox)
        predicted_categories = nearest_neighbour_classify(train_image_feats, train_labels, test_image_feats, 5, 'hamming');
    case 'support vector machine'
        predicted_categories = svm_classify(train_image_feats, train_labels, test_image_feats, .00001);
    case 'convolutional neural network'
        predicted_categories = cnn_classify(train_image_paths, test_image_paths, 'googleplaces');
end
